---
name: execute-phase
description: Execute the current phase — the main computational chemistry research loop
---
...
Read `{GCD_INSTALL_DIR}/specs/workflows/execute-phase-workflow.md` first. Do NOT improvise.
...
5. Verifies calculation outputs after each wave
6. Runs post-phase verification
7. Handles gap closure if verification fails

### Pre-execution
1. Load state via gcd-state
2. Check conventions via gcd-conventions
...
2. For each plan/task in the wave (parallel):
   a. Spawn gcd-executor subagent with task context
...
4. Run inter-wave verification if configured (convergence/convention checks)
5. Update STATE.md

### Post-Phase Verification
After all waves complete:
1. Spawn gcd-verifier with all phase artifacts
...
- Calculation failure: diagnose (SCF convergence, geometry explosion, etc.) and retry with adjusted parameters
