...
Adapted from GPD's git_ops.py for computational chemistry research.
"""
...
def check_no_large_binary(layout: ProjectLayout, files: list[str]) -> list[str]:
    """Check that large binary calculation outputs are not staged."""
    violations = []
    binary_extensions = {".chk", ".fchk", ".rwf", ".cube", ".wfn", ".wfx",
                         ".trj", ".xtc", ".dcd", ".trr", ".gro"}
    for f in files:
        path = layout.root / f
        if path.suffix in binary_extensions:
            violations.append(f"Binary calculation file staged: {f}")
    return violations
...
def run_pre_commit_checks(layout: ProjectLayout, files: list[str]) -> list[str]:
    ...
    violations.extend(check_no_large_binary(layout, files))
    return violations
...
def find_partial_completion(
    ...
    Searches git log for [gcd] commits matching the phase/plan.
