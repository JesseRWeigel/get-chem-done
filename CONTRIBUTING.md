# Contributing to get-chem-done

Thank you for your interest in contributing to **get-chem-done** — an AI copilot for autonomous computational chemistry research.
...
git clone https://github.com/JesseRWeigel/get-chem-done.git
cd get-chem-done
...
Protocols live in `src/gcd/specs/references/protocols/`. Each file encodes a
domain methodology that agents follow during execution.

Existing protocols:
  - `dft-workflow-protocols.md`
  - `md-protocols.md`

To add a new one:

1. Create a Markdown file in `src/gcd/specs/references/protocols/`, e.g.
   `DFT-workflow-protocols.md`.
...
Agents are Markdown files with YAML frontmatter in `src/gcd/agents/`.
...
1. Create `src/gcd/agents/gcd-basis-set-advisor.md`.
2. Include the required YAML frontmatter:

   ```markdown
   ---
   name: gcd-basis-set-advisor
   ...
   <role>
   You are the **GCD Basis Set Advisor** — ...
   </role>
   ```
...
Verification predicates live in `src/gcd/core/kernel.py`. Each check is a
function that inspects evidence and returns a `CheckResult`.

To add a new check:

1. Open `src/gcd/core/kernel.py`.
2. Add a predicate function:

   ```python
   def check_convergence_validation(evidence: dict[str, Any]) -> CheckResult:
       """Checks that scf calculations have properly converged."""
       # Inspect evidence...
       passed = ...  # your logic
       return CheckResult(
           check_id="convergence_validation",
...
1. Create a Python module under `src/gcd/mcp/`, e.g.
   `src/gcd/mcp/basis-set-advisor_server.py`.
2. Implement the server using the MCP SDK:

   ```python
   from mcp.server import Server

   server = Server("gcd-basis-set-advisor")
...
get-chem-done follows the **Get-Paper-Done (GPD)** architecture — a shared framework
